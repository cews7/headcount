...
class HeadcountAnalystTest < Minitest::Test
  ...
  def test_it_calculates_top_statewide_growth_single_leader_grade_3_subject_math
    ...
    assert_equal 0.3, ha.top_statewide_test_year_over_year_growth(grade: 3, subject: :math).last
  end

  def test_it_calculates_top_statewide_growth_single_leader_grade_8_subject_math
    load_data_with_statewide_tests
    ha = HeadcountAnalyst.new(dr)

    assert_equal "COTOPAXI RE-3", ha.top_statewide_test_year_over_year_growth(grade: 8, subject: :reading).first
    assert_equal 0.13, ha.top_statewide_test_year_over_year_growth(grade: 8, subject: :reading).last
  end

  def test_it_calculates_top_statewide_growth_single_leader_grade_3_subject_writing
    load_data_with_statewide_tests
    ha = HeadcountAnalyst.new(dr)

    assert_equal "BETHUNE R-5", ha.top_statewide_test_year_over_year_growth(grade: 3, subject: :writing).first
    assert_equal 0.148, ha.top_statewide_test_year_over_year_growth(grade: 3, subject: :writing).last
  end
